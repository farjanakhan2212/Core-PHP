...
class Customer extends Model implements JsonSerializable{
	...
	public static function filter($name){
		...
		$result=$db->query("select id,name,mobile,email,created_at,updated_at,address,photo from {$tx}customers where name like '%$name%'");
		$data=[];
		while($customer=$result->fetch_object()){
			$data[]=$customer;
		}
...
";
	...
	static function html_select($name="cmbCustomer"){
		global $db,$tx;
		//$html=Form::select(["name"=>$name,"table"=>"{$tx}customers"]);
