...
class District extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}districts(name,division_id)values('$this->name','$this->division_id')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}districts set name='$this->name',division_id='$this->division_id' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,division_id from {$tx}districts");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,division_id from {$tx}districts $criteria limit $top,$perpage");
		$data=[];
		while($district=$result->fetch_object()){
			$data[]=$district;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,division_id from {$tx}districts where id='$id'");
